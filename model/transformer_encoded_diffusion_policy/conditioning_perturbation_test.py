...
def condition_perturbation_test(base_data_folder, num_samples=5, device='cpu'):
    ...
    for row_idx, traj_idx in enumerate(indices):
        ...
        for cond_name, cond_data in condition_sets.items():
            with torch.no_grad():
                # We do Reconstruction (Inverse -> Inverse) to test if conditioning works natively
            ...
            # Denormalize
            pred_seq_raw = denormalize_data(pred_seq.squeeze(0), Y_min_vals, Y_max_vals).cpu().numpy()
            
            # Apply Savitzky-Golay Filter to smooth the DDPM wiggles
            pred_seq_smoothed = savgol_filter(pred_seq_raw, window_length=15, polyorder=3, axis=0)
            predictions[cond_name] = pred_seq_smoothed
        ...
        for col_idx in range(d_y2):
            ...
            for cond_name, pred_smoothed in predictions.items():
                c_color = colors[cond_name]
                
                # Plot Predicted Line
                ax.plot(time_steps, pred_smoothed[:, col_idx], 
    ...
    plt.suptitle(f"Conditioning Perturbation Test (TEDP)\nModel ID: {run_id}", fontsize=16)
    plt.tight_layout()
